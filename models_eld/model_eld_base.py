...
def create_base_eld_model(
    ...
    error_df = pd.read_csv(
        f"{rootpath}/data/results/base_model_eld_errors.csv",
        index_col="building-tower-season",
    )
    error_df.loc[f"{building_name}{tower_number}_{season}"] = [rmse, mae, mae_sd]
    error_df.to_csv(f"{rootpath}/data/results/base_model_eld_errors.csv")
    ...
    fig.show()
    fig.write_html(
        f"{rootpath}/plots/prepared_models/{building_name.lower()}_{season}_eld.html"
    )

    # save the model
    print(model.summary())
    model.save(
        f"{rootpath}/models_saved/base_models/{building_name.lower()}{tower_number}_{season}_eld/"
    )
