...
def create_base_model(
    ...
    model_prep.save_base_errors(
        "GRU", building_name, tower_number, season, rmse, mae, mae_sd, training_time
    )
    ...
    fig.update_layout(
        title=f"{building_name} Tower {tower_number} GRU Model Results",
        xaxis_title="time",
        yaxis_title=target,
    )
    fig.show()
    fig.write_html(
        f"{rootpath}/results/plots/prepared_models/{building_name.lower()}_{season}_gru.html"
    )

    # save the model
    print(model.summary())
    model.save(
        f"{rootpath}/results/models_saved/base_models/{building_name.lower()}{tower_number}_{season}_gru/"
    )
