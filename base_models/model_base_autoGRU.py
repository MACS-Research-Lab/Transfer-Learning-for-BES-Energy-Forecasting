...
def create_base_model(
    ...
    fig.update_layout(
        title=f"{building_name} Tower {tower_number} GRU Autoencoder Model Results",
        xaxis_title="time",
        yaxis_title=target,
    )
    fig.show()
    fig.write_html(
        f"{rootpath}/results/plots/prepared_models/{building_name.lower()}_{season}_autoGRU.html"
    )

    # save the model
    print(model.summary())
    model.save(
        f"{rootpath}/results/models_saved/base_models/{building_name.lower()}{tower_number}_{season}_autoGRU/"
    )
