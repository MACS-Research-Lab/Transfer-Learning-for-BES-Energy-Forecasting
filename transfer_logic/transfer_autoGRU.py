...
def transfer_autoGRU(
    ...
    if finetuning_percentage == 0:
        ...
        model = keras.models.load_model(
            f"../results/models_saved/base_models/{from_building_name.lower()}{from_tower_number}_{from_season}_autoGRU/"
        )
    ...
    # if finetuning is required
        ...
        model_path = f"{rootpath}/results/models_saved/autoGRU_ft/{from_building_name.lower()}{from_tower_number}{from_season}_to_{to_building_name.lower()}{to_tower_number}{to_season}_ft{int(finetuning_percentage*100)}_seed{shuffle_seed}/"
        ...
        # if model finetuning has not been done, finetune a base model
            ...
            model = keras.models.load_model(
                f"{rootpath}/results/models_saved/base_models/{from_building_name.lower()}{from_tower_number}_{from_season}_autoGRU/"
            )
    ...
    def display_transfer_results():
        ...
        fig.update_layout(
            title=f"{from_building_name} Tower {from_tower_number} {from_season} model used on {to_building_name} Tower {to_tower_number} {to_season} ({finetuning_percentage*100}% fine-tuning) GRU-Autoencoder Model Results",
            xaxis_title="time",
